...
(* check-expect : outputs either check_result of Test_Passed, or of Test_Failed
 * of Actual_Result(code), Expected_Result(expect) *)
(* updated by baw 10/10/13 *)
type 'a result = 
| Actual_Result of 'a 
...
type 'a check_result = Test_Passed | Test_Failed of 'a result * 'a result;;
...
let check_error (input : int -> 'a) (expect : string) : 'a check_result =
  try ignore (input 0);
  Test_Failed(Actual_Result(input 0), Expected_Error(expect)) with 
  | (Failure err) when err = expect -> Test_Passed
  | (Failure err) -> Test_Failed(Actual_Error(err), Expected_Error(expect));;
